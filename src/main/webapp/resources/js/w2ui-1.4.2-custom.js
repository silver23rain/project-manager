...
var w2utils = (function () {
    ...
    function scrollBarSize () {
        ...
        //leftiejy : MSIE에서 scrollbar 크기 이상으로 수정
...
(function () {
    var w2grid = function(options) {
        ...
        this.recid   = null;
        this.parser  = null;
        this.maxHeight = 270;		// added by Kisuk. default 270
        this.expandMode = false;	// added by leftie
        // events
        this.onAdd              = null;
        this.onEdit             = null;
    ...
    w2grid.prototype = {
        ...
        buttons: {
            ...
            'search-go': { type: 'check',  id: 'w2ui-search-advanced', caption: w2utils.lang('검색'), hint: w2utils.lang('Open Search Fields') },
        ...
        select: function () {
            ...
            for (var a = 0; a < arguments.length; a++) {
                ...
                if (this.selectType == 'row') {
                    ...
                    recEl.find('.w2ui-grid-select-check').prop("checked", true).attr("checked", true);
                    selected++;
                } else {
        ...
        unselect: function () {
            ...
            for (var a = 0; a < arguments.length; a++) {
                ...
                if (this.selectType == 'row') {
                    ...
                    recEl.find('.w2ui-grid-select-check').prop("checked", false).attr("checked", false);
                    unselected++;
                } else {
                    var col  = arguments[a].column;
        ...
        keydown: function (event) {
            ...
            switch (key) {
                ...
                case 91: // cmd key
                    if (empty) break;
                    var text = obj.copy();
                    //leftiejy : ie에서 해당 textarea가 화면에 나타나는 현상이 있어 display: none 을 추가함
                    $('body').append('<textarea id="_tmp_copy_data" '+
                        '   onpaste="var obj = this; setTimeout(function () { w2ui[\''+ obj.name + '\'].paste(obj.value); }, 1);" '+
                        '   onkeydown="w2ui[\''+ obj.name +'\'].keydown(event)"'+
                        '   style="display:none position: absolute; top: -100px; height: 1px; width: 1px">'+ text +'</textarea>');
        ...
        expand: function (recid) {
        	if(this.expandMode == false){
        		//leftiejy
        		console.log("ERR : expandMode가 false인 경우 expand시 scroll의 문제가 발생합니다.");
        	}
            var rec = this.get(recid);
            var obj = this;
        ...
        refresh: function () {
        	var self = this;
        	// Added By Kisuk. 그리드의 최대 높이를 리프레시 하기 전에 지정해주기 위해서.
        	// grid name은 무조건 테이블의 id와 같아야 한다.
        	
        	// Modifed by leftie. w2Grid의 scroll 버그를 회피하기 위해 expandMode의 경우 maxHeight를 항상 반영한다.  
        	
        	// this.records.length의 경우 search 시점에서 실제 렌더링할 레코드 개수가 아닌 전체 개수를 계속 반환하기 때문에
        	// 실제 렌더링 요소의 개수인 this.total을 사용하여 비교해야 합니다. 다만, 렌더링 파이프라인 단계 중 실제로 
        	// 렌더링 할 레코드가 존재함에도 불구하고 total 값이 세팅되기 전이라 0인 경우의 예외도 판단하여야 합니다.
        	// 위 예외 케이스는 edit_path의 2개의 grid init 순서에 기인합니다. (edit_path 24line)
        	var needFixedBody = function(base) {
                var height 	  = parseInt(base + 1) * parseInt(self.recordHeight);
        	...
        	if (this.expandMode == false){
	        	if (needFixedBody(this.total)) {
	        		var $table = $("#" + this.name);
	        		if ($table.length == 0) {
	        			console.log("ERROR : grid의 name은 table tag의 id와 동일해야 합니다.");
	        		} else {
                    ...
                    if (colg.master === true) {
                        ...
                        html += '<td class="w2ui-head '+ sortStyle +'" col="'+ ii + '" rowspan="2" colspan="'+ (colg.span + (i == obj.columnGroups.length-1 ? 1 : 0) ) +'" '+
                                ...
                                '    <div class="w2ui-col-group w2ui-col-header '+ (sortStyle ? 'w2ui-col-sorted' : '') +'">'+
                                	(col.sortable ? '<div class="w2ui-sort-up sort-default"></div><div class="w2ui-sort-down sort-default"></div>' : '') +
        ...
        getRecordsHTML: function () {
            ...
            if (buffered > MAX_BUFFER_SIZE) this.show_extra = 30; else this.show_extra = MAX_BUFFER_SIZE;
            var records  = $('#grid_'+ this.name +'_records');
            ...
            html += '<tr id="grid_'+ this.name + '_rec_top" class="rec_top" line="top" style="height: '+ 0 +'px">'+
                    '    <td colspan="200"></td>'+
                    '</tr>';
            for (var i = 0; i < limit; i++) {
                html += this.getRecordHTML(i, i+1);
            }
            html += '<tr id="grid_'+ this.name + '_rec_bottom" class="rec_bottom" line="bottom" style="height: '+ ((buffered - limit) * this.recordHeight) +'px">'+
                    '    <td colspan="200"></td>'+
                    '</tr>'+
        ...
        scroll: function (event) {
			...
			// 브라우저 환경별로 jquery height()를 가져오는데에 오차가 있어 recordHeight의 1/4을 버퍼로 추가함. by Kisuk.
            if (records.height() + (this.recordHeight / 4) < buffered * this.recordHeight && records.css('overflow-y') == 'hidden') {
            ...
            if (!url && (!this.fixedBody || this.total <= MAX_BUFFER_SIZE)) return;
            // regular processing
        ...
        getCellHTML: function (ind, col_ind, summary) {
            ...
            if (typeof col.render != 'undefined') {
                ...
                //Heung
                //need encoding for converting < to &lt;
                ...
                if (!this.show.recordTitles) {
                    ...
                    data = '<div title="'+ w2utils.stripTags(title) +'" style="'+ addStyle +'">'+ data +'</div>';
                }
            }
